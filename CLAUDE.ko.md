...
뒤처지는 것으로 드러난 뒤 작성된, 우선순위가 매겨진 성능 개선 계획입니다. 이 저장소에서 성능
관련 작업을 이어받는다면 우선순위를 처음부터 다시 도출하지 말고 거기서 시작하세요 — 기대
효과 대비 노력 순으로 정렬되어 있고, 각 항목이 왜 그렇게 범위가 잡혔는지도 설명합니다.
...
- **Phase 16(PLAN.md 실행 — P1부터 P5까지) — 완료.** PLAN.md가 제안한 모든 것을 실행해달라는
  사용자의 명시적 요청에 따라 진행했으며, 두 항목(P3, 그리고 P4의 후반 반복들)은 그 항목 자신의
  문구가 "프로파일링 데이터 없이는 시도하지 말라"고 되어 있었음에도 사용자가 그래도 해달라고
  요청해서 실행했습니다. 전체 결과, 수치, 재현 방법은 `ReadMe.md`의 벤치마크 섹션과 PLAN.md
  자체에 있습니다(각 항목의 기록은 별도의 "실제로 한 일" 로그로 남기지 않고 그 자리에서 실제로
  일어난 일을 담도록 다시 썼습니다) — 이 항목은 그 요약본과 거기 없는 내용만 담습니다.
  - **종합 결과**: 루프백 13.8K → 58.0K req/s(약 4.2배), Docker 네트워크 스택 벤치마크
    11.2K → 35.9K req/s(약 3.2배) — nhttpd는 이제 둘 다에서 Apache를 앞서고 nginx와의 남은
    격차 대부분을 좁혔습니다(루프백에서는 대략 nginx 처리량의 절반, Docker에서는 3분의 2,
    예전엔 각각 10분의 1과 6분의 1이었습니다).
  - **P1(`sendfile(2)` 고속 경로)과 P2(스레드 풀 왕복 감소)**는 함께 반영됐습니다; `io::
    file_stream`은 `native_fd()`를, `platform::socket_handle`은 `send_file()`/
    `supports_send_file()`을 얻었습니다(Windows는 정확하게 `false`를 반환합니다 —
    `TransmitFile`은 이 리액터의 평범한 읽기/쓰기가 일부러 쓰지 않는 진짜 IOCP 완료 모델이
    필요한데, 이 코드베이스의 다른 모든 것만큼 철저하게 오버랩드 완료의 엣지 케이스를 검증할
    수 없는 채로는 감수할 가치가 없는 위험입니다), `overlay`/`single_file`은 `wants()`와
    `handle()` 사이에서 요청을 두 번 `stat()`하지 않게 됐습니다.
  - **P3(커스텀 코루틴 프레임 할당자)는 구현·벤치마크 후 반려됐습니다** — glibc 자체의
    `tcache`(이미 거의 같은 일을 함) 대비 측정 가능한 개선이 없었습니다. `include/nhttp/
    async/task.hpp`는 정확히 Phase 15의 수정만 남은 상태로 돌아갔습니다.
  - **P4(`thread_pool`의 작업 큐)는 한 번이 아니라 세 번의 실제 반복을 거쳤으며**, 이번
    phase에서 가장 교훈적인 결과입니다: `std::counting_semaphore` 기반 락-프리 큐는 *약 2배
    느리게* 측정됐습니다; 락-프리 큐와 유휴 워커를 깨우기 위한 순수 뮤텍스+condvar의 조합이
    이를 고쳤지만, `blocking_pool_size`가 일부러 과다 프로비저닝되지 않았을 때만 그랬습니다 —
    이 문서가 예전에 권장했던 큰 풀 크기(64)에서는 CPU 코어 8개 위에서 스레드 64개가 경합하며
    락-프리 큐가 절약한 것보다 컨텍스트 스위칭 오버헤드로 훨씬 많이 잃었습니다(직접 확인:
    *예전* 뮤텍스+`std::queue` 설계는 풀 크기 64에서 8로 갈 때 *더 빨라졌고*, 락-프리 설계는
    같은 방향으로 *더 느려졌습니다* — 반대 경향이고 둘 다 실측입니다); 라이브러리 자체의
    손대지 않은 기본값(`blocking_pool_size` = 4)에서는 약 69K req/s를 기록해 이번 phase 전체
    최고 결과였습니다. `blocking_pool_size`는 이제 수동 튜닝이 전혀 필요 없습니다 — 이
    저장소가 예전에 권장하던 것의 완전한 반전입니다. 후속 단순화(깨우기 알림 주변의 "누가
    대기 중인가" 원자적 게이트 제거 — glibc의 `condition_variable::notify_one()`이 대기자가
    없으면 이미 자체적으로 futex-wake syscall을 건너뛰므로)는 측정상 동등해서 더 단순한
    코드를 위해 채택했습니다. `include/nhttp/async/detail/mpmc_queue.hpp`(Dmitry Vyukov의
    bounded MPMC 설계)를 전용 스트레스 테스트(`tests/unit/test_mpmc_queue.cpp` — 프로듀서
    8개 × 컨슈머 4개 × 각 2만 개 항목을 일부러 작게 잡은 큐로 흘려보내며 모든 항목이 정확히
    한 번씩 전달되는지 확인)와 함께 추가했으며, 매 반복마다 벤치마크하기 전에 전체 스위트와
    함께 ThreadSanitizer 아래서 깨끗함을 검증했습니다.
  - **P5(윈도우 슬라이딩, 메모리 매핑 파일 읽기 경로)**는 `platform::file_mapping`을
    추가했습니다 — 파일 전체를 매핑하는 대신 4&nbsp;MiB 슬라이딩 윈도우로 제한됩니다(멀티
    기가바이트 파일을 작은 바이트 `Range` 요청 하나를 위해 통째로 매핑하면 잘해야 주소
    공간/페이지 테이블 설정을 낭비하고, 32비트 대상에서는 아예 고갈시킬 수 있습니다). 매핑은
    `open()`에서 즉시가 아니라 스트림의 실제 첫 `read()` 호출에서 **지연** 생성됩니다 — 즉시
    생성하는 버전을 먼저 시도했다가, P1의 sendfile 경로가 존재하는 한 `read()`를 아예 호출하지
    않는 흔한 전체-파일-GET 경우를 그 요청이 절대 쓰지 않을 매핑의 비용을 치르게 만들어
    측정 가능하게 퇴보시켰습니다.
  - **이번 phase 작업과 무관하게 발견되어 바로잡은 실제 이상 현상**: 이 세션의 변경과 무관한
    파일 두 개(`.gitignore`, `LICENSE`)와 레거시 벤치마크 자산 하나(`benchmark/
    benchmark-1.jmx`)가, P4의 설계를 P4 적용 전 기준선과 A/B 비교하는 데 썼던 `git stash`/
    `stash pop` 왕복의 부작용으로 줄바꿈만 바뀐(LF→CRLF) diff를 갖게 됐습니다. 내용 변경은
    없었습니다. 무관한 노이즈로 커밋에 끌고 가는 대신 커밋 전에 `git checkout`으로
    되돌렸습니다.
- 계획에 남은 것은 QUIC/HTTP-3(보류, 결정 #8 참고), Phase 12가 기록한 Windows에서의 OpenSSL
  빌드 환경 공백, 그리고 Phase 16 이후 PLAN.md가 아직 열려 있다고 기록한 것들뿐입니다(PLAN.md의
  P5 항목 자체에 좁은 범위의 후속 작업 몇 가지가 적혀 있습니다 — P1을 위한 Windows
  `TransmitFile`, 그리고 실제로 `perf`를 돌릴 수 있는 호스트에서 P3/P4 관련 질문을 제대로
  프로파일링하는 것). 이 저장소의 향후 작업은 여기서부터 시작합니다 — 위의 모듈 맵과 빌드 안내,
  사용자 대상 API 투어는
  `ReadMe.md`, 전체 성능 작업 기록은 `PLAN.md`를 참고하세요.
