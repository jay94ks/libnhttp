...
세 번째 문서는 과거가 아니라 앞을 봅니다: [`PLAN.md`](PLAN.ko.md)는 Phase 15의 벤치마크
라운드(아래 해당 phase 항목 참고)에서 nhttpd가 정적 파일 서빙 시 nginx/Apache에 상당히
뒤처지는 것으로 드러난 뒤 시작된, 아직 남은 성능 후속 작업만 담는 살아있는 할 일
목록입니다. 항목이 완료되면 거기서 지워지고, 무엇을 시도했고 측정했고 채택하거나 반려했는지의
전체 이야기는 이 파일의 phase 로그로 영구히 옮겨집니다(Phase 16 참고). 이 저장소에서 성능
관련 작업을 이어받는다면 먼저 `PLAN.md`에서 아직 남은 것을, 이미 끝난 것의 이력은 이 파일의
phase 로그를 확인하세요.
...
- **Phase 15 (nginx/Apache 대비 벤치마크) — 완료.** nginx와 Apache 2 대비 벤치마크를 돌려서
  `ReadMe.md`에 결과를 반영해달라는 사용자 요청에 따름. 지속 부하 벤치마크(테스트당 수십 건만
  보내던 기존 테스트 스위트로는 전혀 드러나지 않던 종류)에서 실제 버그 두 개를 찾아 고쳤고,
  지금 `main`에 반영되어 있습니다:
  1. **`task<T>`가 이 재작성의 첫 커밋부터 매 `co_await`마다 자기 코루틴 프레임을
     누수시키고 있었습니다.** `operator co_await() &&`가 콜리의 코루틴 핸들을 절대 파괴하지
     않는 일회용 `awaiter`에게 넘기면서 task 자신의 `handle_`을 비워버려서, 완료된 프레임을
     파괴하는 곳이 어디에도 없었습니다. `task<T>`는 거의 모든 비동기 함수의 반환 타입이므로
     어디서든 누수가 발생했습니다. 정적 파일 23만 3천 요청 동안 RSS가 5.6&nbsp;MB →
     5.78&nbsp;GB로 자라는 걸로 발견했고, 독립 재현(아무 일도 안 하는 task 200만 번
     await → 약 125&nbsp;MB 누수)으로 확인했습니다. **수정**: `handle_`을 더 이상 비우지
     않아서, `co_await` 표현식의 임시 객체 수명이 끝날 때 task 자신의 소멸자가 프레임을
     해제하도록 함 — cppcoro 스타일 task의 표준 패턴입니다. 검증: 113/113 테스트 변경 없이
     통과, 지속 부하에서 RSS 평탄.
  2. **연결을 중간에 리셋하는 클라이언트 하나가 프로세스 전체를 죽였습니다.** 이미 리셋된
     소켓에 `write()`하면 `SIGPIPE`가 발생하는데, 기본 처리는 디버거/새니타이저가 잡을 것도
     없이 프로세스 전체를 즉시 종료시키는 것입니다 — `strace`로 근본 원인을 찾았고, `wrk`가
     커넥션 풀을 정리하는 순간 `+++ killed by SIGPIPE +++`가 찍혔습니다. **수정**: POSIX에서
     `listener`의 생성자에 `std::signal(SIGPIPE, SIG_IGN)` 추가(Windows는 대신
     `WSAECONNRESET`을 반환, 시그널 자체가 없음). 검증: 113/113 통과, 30초 안에 죽던
     벤치마크가 이제 42만 6천 요청을 죽지 않고 처리.
  - **방법론**: 이 머신 WSL Ubuntu에 `apt`로 설치한 nginx 1.24와 Apache 2.4.58(event MPM,
    `MaxRequestWorkers` 150→800 튜닝)이 동일한 10&nbsp;KB 정적 파일 서빙; `wrk -t8 -c200
    -d30s --latency`를 같은 호스트 루프백과 Docker Compose 스택(`benchmark/docker/` — 루프백이
    건너뛰는 실제 브리지 네트워크 커널 경로를 거침) 양쪽에서 실행. 전체 수치는 `ReadMe.md`의
    벤치마크 섹션 참고.
  - **결과, 정직하게**: 두 수정 후에도 nhttpd는 이 마이크로벤치마크에서 nginx/Apache에 크게
    뒤처졌습니다(13.8K req/s 대 nginx 139K, Apache 38.6K) — 실제 미최적화 아키텍처 격차이지
    (정적 파일 요청마다 블로킹 스레드 풀을 여러 번 왕복하는 반면 nginx는 `sendfile()` 한
    번으로 처리) 버그는 아닙니다. 격차 해소는 아래 Phase 16 참고.
- **Phase 16(PLAN.md 실행 — P1부터 P5까지) — 완료.** 사용자 요청에 따라 성능 계획의 모든
  항목을 실행했으며, 두 항목(P3, P4의 일부)은 "프로파일링 데이터 없이는 시도하지 말라"는
  자신의 문구에도 불구하고 실행했고 각 결과는 A/B 벤치마크로 판단했습니다. 전체 수치는
  `ReadMe.md`의 벤치마크 섹션 참고; PLAN.md 자체는 이제 아직 남은 것만 추적합니다(P1의 Windows
  `TransmitFile`, 이 WSL2 커널에서는 불가능한 실제 `perf` 프로파일링). 종합 결과: 루프백
  13.8K → 58.0K req/s(약 4.2배), Docker 네트워크 스택 벤치마크 11.2K → 35.9K req/s(약
  3.2배) — nhttpd는 이제 둘 다에서 Apache를 앞섭니다.
  - **P1/P2**: `sendfile(2)` 고속 경로(`io::file_stream::native_fd()`, `platform::
    socket_handle::send_file()`/`supports_send_file()` — Windows는 `false` 반환, 검증
    불가능한 `TransmitFile`/IOCP 통합의 위험을 감수하는 대신 보류) 및 `overlay`/`single_file`의
    `wants()`↔`handle()` 사이 중복 `stat()` 제거.
  - **P3**: `task<T>`용 커스텀 코루틴 프레임 할당자 — 구현·벤치마크했으나 glibc `tcache` 대비
    측정 가능한 개선 없음. 반려; `task.hpp`는 Phase 15 수정만 남은 상태.
  - **P4**: `thread_pool`의 작업 큐, 세 번의 반복. `std::counting_semaphore` 기반 락-프리 큐는
    **약 2배 느림**으로 측정(워커가 이미 바빠도 매 dequeue마다 동기화 비용 지불, 대체 대상인
    뮤텍스는 그러지 않았음). 락-프리 큐 + 유휴 워커 깨우기 전용 뮤텍스/condvar 조합이 이를
    고쳤지만 `blocking_pool_size`가 과다 프로비저닝되지 않았을 때만 — 코어 8개에 스레드 64개가
    경합하면 락-프리 큐가 절약한 것보다 컨텍스트 스위칭으로 잃는 게 컸고, 라이브러리 기본값(4)
    에서는 **약 69K req/s**로 이번 phase 최고 기록 — `blocking_pool_size`는 이제 수동 튜닝이
    필요 없습니다(이 저장소의 예전 권장과 정반대). 후속 단순화(깨우기 알림의 원자적 대기자
    게이트 제거 — glibc `notify_one()`이 이미 대기자 없으면 자체 syscall을 건너뜀)는 측정상
    동등해서 더 단순한 코드로 채택. 신규: `include/nhttp/async/detail/mpmc_queue.hpp`(Vyukov의
    bounded MPMC 설계)와 전용 스트레스 테스트; 매 반복 벤치마크 전에 ThreadSanitizer로 검증.
  - **P5**: `platform::file_mapping` — `io::file_stream`용 메모리 매핑 읽기 경로, 파일 전체가
    아니라 4&nbsp;MiB 슬라이딩 윈도우로 제한(거대 파일 전체 매핑은 주소 공간 낭비/고갈 위험),
    스트림의 첫 `read()` 호출에서 **지연** 생성. 즉시 생성 버전을 먼저 시도했다가, P1의
    sendfile 경로가 존재하는 한 `read()`를 아예 호출하지 않는 흔한 전체-파일-GET 경우를
    측정 가능하게 퇴보시켜서 지연 방식으로 바꿈.
