...
// include/nhttp/**. keeps sockaddr/socklen_t out of every public header.
...
namespace nhttp::platform::posix_detail {

	inline socklen_t endpoint_to_sockaddr(const endpoint& ep, sockaddr_storage& out) noexcept {
	...
	inline std::optional<endpoint> endpoint_from_sockaddr(const sockaddr* addr, socklen_t len) noexcept {
